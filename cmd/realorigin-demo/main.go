...
var rootCmd = &cobra.Command{
	Use:   "realorigin-demo",
	Short: "a working example of the substrate over plain HTTP",
	Long: "realorigin-demo runs the two halves the design requires: the app origin,\n" +
		"which holds the transport, and the browse origins, which serve the worker\n" +
		"and the shell and never see any content.\n\n" +
		"Both names resolve to loopback with no DNS, and browsers treat *.localhost\n" +
		"as a secure context, so a service worker registers over plain HTTP with no\n" +
		"certificate. Two names are still needed: one host and port would be one\n" +
		"origin, and the content would be inside the app.",
	SilenceUsage:  true,
	SilenceErrors: true,
...
func init() {
	f := rootCmd.Flags()
	f.StringVar(&appAddr, "app", "127.0.0.1:7999", "listen address for the app origin")
	f.StringVar(&browseAddr, "browse", "127.0.0.1:7998", "listen address for the browse origins")
	f.StringVar(&suffix, "suffix", ".demo.localhost", "browse-origin domain suffix")
}
